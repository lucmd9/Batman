...
async def بحث(event):
    ...
    catevent = await edit_or_reply(event, "`لكيته....`")
...
async def يوت(event):
    ...
    cat = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    catevent = await edit_or_reply(event, "`لكيتهاا....`")
    video_link = await yt_search(str(query))
    if not url(video_link):
        return await catevent.edit(
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await edit_or_reply(
                catevent, "**Error:** شيل البلوك من البوت او اشترك بالقنوات الاجباري بالبوت وامسح المحادثه انت والبوت وجرب
